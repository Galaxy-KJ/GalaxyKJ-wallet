...
const MetricsCard = memo(({ 
  ...
  return (
    <div className={`rounded-lg border p-3 backdrop-blur-sm ${
      status ? statusColors[status] : 'text-gray-300 bg-gray-900/50 border-gray-700'
    }`}>
...
const BundleHealthIndicator = memo(({ report }: { report?: BundleAnalysisReport }) => {
...
const OptimizationRecommendations = memo(({ report }: { report?: BundleAnalysisReport }) => {
  if (!report || report.recommendations.length === 0) return null;

  const criticalRecs = report.recommendations.filter(r => r.severity === 'critical').slice(0, 3);

  return (
    <div className="rounded-lg border border-gray-700 bg-gray-900/50 p-3 backdrop-blur-sm">
      <h4 className="text-sm font-medium text-gray-300 mb-2">Top Optimizations</h4>
  ...
  return (
    <div className="grid grid-cols-2 gap-2 mb-4">
      <MetricsCard
        ...
        value={totalLoadTime}
        unit="ms"
        status={totalLoadTime < 1000 ? 'good' : totalLoadTime < 2000 ? 'warning' : 'critical'}
        icon={Clock}
      />
      <MetricsCard
        title="Render Time"
        ...
        unit="ms"
        status={totalRenderTime < 16 ? 'good' : totalRenderTime < 32 ? 'warning' : 'critical'}
        icon={Zap}
      />
      <MetricsCard
        title="Memory"
        ...
        unit="MB"
        status={avgMemoryUsage < 50 * 1024 * 1024 ? 'good' : avgMemoryUsage < 100 * 1024 * 1024 ? 'warning' : 'critical'}
        icon={Activity}
      />
      <MetricsCard
        title="Cache Hit"
        ...
        unit="%"
        status={avgCacheHitRate > 0.8 ? 'good' : avgCacheHitRate > 0.5 ? 'warning' : 'critical'}
        icon={Package}
      />
    </div>
  );
...
export const PerformanceMonitorUI = memo(function PerformanceMonitorUI({
  ...
  return (
    <div 
      className={`fixed ${positionClasses[position]} z-50 max-w-sm transition-all duration-300 ${
